...
// Inline color swatches + picker for the hex colours in a snippet: C# writes them as
// Color.FromHex("...") / Color.Parse("..."), TSX as a plain string prop (TextColor="#FFFFFF",
// or "#1F2937" assigned in code) - one provider per language, same picker.
...
window.fiddleRegisterColorPicker = function () {
    ...
    // C#: only inside a Color factory call - a bare "#..." string there is usually not a colour.
    // TSX: any quoted hex literal, because that IS how a colour is written in a tag or an assignment.
    const PATTERNS = {
        csharp: /Color\.(?:FromHex|Parse)\(\s*"(#[0-9a-fA-F]{3,8})"\s*\)/g,
        typescript: /["'`](#[0-9a-fA-F]{3,8})["'`]/g,
    };
    ...
    for (const [lang, re] of Object.entries(PATTERNS))
        monaco.languages.registerColorProvider(lang, provider(re));
};
...
// lang: 'tsx' for a React snippet, anything else for C# — the API answers with an id that says so.
// js: for a React snippet, the transpiled module — stored with the share so the player runs it
// without any compiler.
window.fiddleShareShort = async function (code, pngBase64, bg, kind, nosave, desc, tags, assetsBase64, lang, js) {
    const r = await fetch(fiddleShareApi(), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...fiddleAuthHeaders() },
        body: JSON.stringify({ code: code, png: pngBase64 || undefined, bg: bg || undefined, kind: kind || undefined, nosave: nosave || undefined, desc: desc || undefined, tags: tags || undefined, assets: assetsBase64 || undefined, lang: lang || undefined, js: js || undefined }),
    });
    if (!r.ok) throw new Error('share api ' + r.status);
    return (await r.json()).url;
};

// The public link of a share, on whatever origin this page is served from.
window.fiddleShareLink = function (id) {
    return location.origin + '/f/' + id;
};

// Read a share back after an in-place edit (undo): same shape as fiddleShareDecode returns.
window.fiddleShareFetch = async function (id) {
    const r = await fetch(fiddleShareApi() + '/' + id, { headers: fiddleAuthHeaders() });
    if (!r.ok) throw new Error('share api ' + r.status);
    return { id: id, code: await r.text() };
};
...
window.fiddleCheckVersion = async function () {
    ...
    try {
        ...
        card.innerHTML = '<div style="font:800 17px ' + font + ';color:#e6edf6;margin-bottom:6px">New version available</div>'
            + '<div style="font:400 13px ' + font + ';color:#8b98ab;line-height:1.5">Publish update ' + m[1] + ' is out — you are running ' + mine + '.<br>Reload to get it. Unsaved editor changes are lost.</div>';
...
function wireFiddleDiagnostics() {
    ...
    window.fiddleRunDiagnostics = function () {
        // The editor can go away under a pending pass - a reload, or leaving /app - and every
        // call on a disposed model throws "Model is disposed!" into the console for nothing.
...
// The editor can go away (back to the feed) while window.fiddle still points at its .NET
// object. Every call on that handle throws "no tracked object with id N" INSIDE Blazor, which is
// what raises the red "An unhandled error has occurred" bar. The page drops the API on the way out
// and registers a fresh one when it comes back.
window.unregisterFiddleApi = function () {
    window.fiddle = null;
    window.fiddleIntelliSense.dotnet = null;
};
...
window.registerFiddleApi = function (dotnetRef) {
    window.fiddle = {
        ...
        setCode: function (code, lang) { return dotnetRef.invokeMethodAsync('ApiSetCode', code, lang || null); },
        ...
        // Put a shared snippet back into its card view (rating, author, Edit) - see ApiShowCard.
        showCard: function () { return dotnetRef.invokeMethodAsync('ApiShowCard'); },
