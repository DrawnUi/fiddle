...
(function () {
    ...
    var S = {
        ...
        sort: 'new', kind: 'all', tag: '', tags: [], lang: '',
    ...
    // Which language a snippet is written in — C# compiled by Roslyn, or TSX drawn by DrawnUi.React.
    // Filters in place like the tag pills; no URL of its own.
    async function selectLang(lang) { S.lang = lang; await load(0); render(); }
    ...
    // The language is written into the id: C# ids are 8 base62 characters, React ids are those
    // 8 behind an "r" (see share() in the worker). Same rule on the server side of this feed.
    function isReactId(id) { return String(id).length === 9 && String(id)[0] === 'r'; }

    function langChip(it) {
        return isReactId(it.id)
            ? '<span class="fc-lang react">React</span>'
            : '<span class="fc-lang csharp">C#</span>';
    }
    ...
    function render() {
        ...
        // Carry the thumbnails across instead of writing over them. Every render throws the old
        // cards away, and a brand-new <img> paints nothing until it is decoded again — which is why
        // the grid blanked for a frame each time, most visibly when this first render replaces the
        // feed the worker put in the HTML (its cards are <a> for crawlers, these are <button>, so
        // the markup can never simply match). A MOVED node keeps its pixels; a fresh one does not.
        var next = document.createElement('div');
        next.innerHTML = html;
        // Keyed by the picture's PATH: the worker writes "/f/{id}/og.png" and this file writes the
        // absolute url, so the raw attributes never match even when it is the same picture. A
        // query string means someone deliberately busted that thumbnail — fetch it.
        var thumbKey = function (img) {
            try {
                var u = new URL(img.getAttribute('src'), location.href);
                return u.search ? null : u.pathname;
            } catch (e) { return null; }
        };
        var mine = {};
        Array.prototype.forEach.call(host.querySelectorAll('.fc-card img[src]'), function (img) {
            var key = thumbKey(img);
            if (key) mine[key] = img;
        });
        Array.prototype.forEach.call(next.querySelectorAll('.fc-card img[src]'), function (img) {
            var key = thumbKey(img);
            var keep = key && mine[key];
            if (keep) img.parentNode.replaceChild(keep, img);
        });
        host.innerHTML = '';
    ...
    function renderTabs() {
        ...
        // Its own pill: ordering and language are separate questions, and one row of five buttons
        // read as if "New" and "React" excluded each other.
        var langs = el('fx-langs');
        if (langs) {
            var lang = function (v) { return S.lang === v ? ' class="on"' : ''; };
            langs.innerHTML =
                '<button' + lang('') + ' data-act="lang" data-lang="" title="Both languages">All</button>' +
                '<button' + lang('csharp') + ' data-act="lang" data-lang="csharp" title="Snippets written in C#, compiled with Roslyn in the browser">C#</button>' +
                '<button' + lang('tsx') + ' data-act="lang" data-lang="tsx" title="Snippets written in TSX, drawn by DrawnUi.React">React</button>';
        }
    }
    ...
    function popHtml(it) {
        ...
        // Everything listed here is open source, whatever the library row calls itself: an item that
        // only offered "Run App" was a black box on a page about reading code.
        var actions =
        ...
        return '<div class="fc-overlay" id="fx-pop-overlay" tabindex="0" data-act="close-pop">' +
            ...
            '<h2 class="fc-pop-title">' + esc(name(it)) + ' ' +
                (it.official === 1 ? '<span class="fc-kind official">preset</span>' : '') + '</h2>' +
            ...
            '<p class="fc-pop-meta">' + (it.author ? 'by ' + esc(it.author) + ' · ' : '') + stars(it.avg) + ' ' +
                (it.avg || 0).toFixed(1) + ' · ' + (it.votes || 0) + ' ' + (it.votes === 1 ? 'vote' : 'votes') + ' · ' + esc(it.id) +
                (it.edited ? ' · edited ' + new Date(it.edited).toLocaleDateString() : '') + '</p>' +
    ...
    function contributeHtml() {
        return '<div class="fc-overlay" data-act="close-contribute">' +
            ...
            '<a class="fc-ghost" href="/app">Create with C#</a>' +
            '<a class="fc-ghost" href="/app/react-welcome">Create with React</a>' +
            '</div></div></div></div>';
    }
    ...
    document.addEventListener('click', function (e) {
        ...
        switch (act) {
            ...
            case 'lang': selectLang(node.getAttribute('data-lang')); return;
            case 'tag': {
    ...
    function revealEditor() {
        ...
        // Same fiddle as the one already mounted: the deep link below is skipped, so the editor
        // would still be in whatever state it was left in - including the middle of an edit. Coming
        // in from the feed is a fresh look at the snippet, so ask for the card view back.
        if (alreadyMounted && here === servedUrl && window.fiddle && window.fiddle.showCard)
            window.fiddle.showCard();
    ...
    // Blazor's router intercepts every same-origin link once the runtime is warm - and the landing
    // warms it on purpose - so a link to a page the SPA does not own (/docs/, /terms/, /privacy/,
    // /skills/) was handed to the editor's catch-all route: a canvas-less editor, a burst of null
    // reference errors in the console, and the visitor still staring at the page they clicked from.
    // Catch those in the capture phase, before either router sees the click, and navigate for real.
    document.addEventListener('click', function (e) {
        if (e.defaultPrevented || e.button !== 0 || e.metaKey || e.ctrlKey || e.shiftKey || e.altKey) return;
        var a = e.target.closest ? e.target.closest('a[href]') : null;
        if (!a || a.target || a.hasAttribute('download') || a.hasAttribute('data-act')) return;
        var to;
        try { to = new URL(a.getAttribute('href'), location.href); } catch (x) { return; }
        if (to.origin !== location.origin) return;
        if (isEditorPath(to.pathname) || isLandingPath(to.pathname.replace(/\/+$/, ''))) return;
        e.preventDefault();
